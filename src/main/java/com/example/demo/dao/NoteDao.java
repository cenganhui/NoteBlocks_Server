...
public class NoteDao {
    ...
    public static void addNote(Note newNote) {
        System.out.println("NoteDao:addNote -- " + newNote);
    ...
    public static void removeNoteByIds(int ids) {
        for (int j = 0; j < allNotes.size(); j++) {
            if (allNotes.get(j).getN_ids() == ids) {
                DbDao.delDataByIds(allNotes.get(j).getN_ids(), allNotes.get(j).getU_name());
                allNotes.remove(j);
    ...
    public static void changeNote(Note newNote) {
        ...
        for (int j = 0; j < allNotes.size(); j++) {
            if (allNotes.get(j).getN_ids() == newNote.getN_ids()) {
...
//        errorNote.setPicturePath(errorDes);
        errorNote.setN_ids(-1);
        errorNote.setN_title(errorDes);
        errorNote.setN_content(errorDes);
        errorNote.setN_times(null);
        errorNote.setN_audioPath(errorDes);
        errorNote.setN_picturePath(errorDes);
        return errorNote;
    }
